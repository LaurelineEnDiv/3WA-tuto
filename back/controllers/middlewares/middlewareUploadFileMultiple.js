...
export default async (req, res, next) => {
  form.parse(req, async (err, fields, files) => {
    ...
    if (err) {
      ...
      if (err.code === "LIMIT_FIELD_SIZE") {
        return res
          .status(400)
          .json({
            error: `Le fichier dépasse la taille maximum autorisée de ${
      ...
      return res
        .status(500)
        .json({ error: "Le fichier ne peut pas être traité." });
    }
    ...
    if (
      ...
      (!Array.isArray(images) && images.size === 0)
    ) {
    ...
    for (const image of images) {
      ...
      if (!fs.existsSync(imageDirectory)) {
        return res
          .status(500)
          .json({ error: `Le dossier ${imageDirectory} n'existe pas.` });
      }
      ...
      try {
        ...
        return res
          .status(500)
          .json({ error: "Le fichier ne peut pas être enregistré." });
      }
