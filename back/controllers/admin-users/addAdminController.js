...
export default async (req, res) => {
  ...
  if (password.length < 8) {
    return res.json({
      response: "votre mot de passe doit contenir au moins 8 caractères",
    });
  } else if (
    nom.length > 255 ||
    prenom.length > 255 ||
  ...
  try {
    ...
    // On hash le password
    const mpdHash = await bcryptjs.hash(password, saltRounds);
    ...
    return res.json({
      response: createUser,
      response: "Votre compte admin est bien créé!",
    });
  } catch (err) {
    console.log(err);
