...
const Compagnie = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => setMembers(orderByOrder(res.data.result)))
      .catch((e) => console.log(e));
  }, []);

  useEffect(() => {
    axios
      .get(`${BASE_URL}/listpartners`)
        ...
        <Fragment>
          {partners.length > 0 &&
            partners.map((partner, i) => (
              <a key={i} href={partner.url} target="_blank">
                {" "}
                {partner.nom} /{" "}
              </a>
            ))}
